...
namespace _3dsGallery.WebUI.Controllers
{
    public class UserController : Controller
    {
        ...
        public ActionResult UserProfile(string login)
        {
            ...
            model.TotalImageCount = db.Picture.Where(x => (x.StorageMigrationStatus == null || x.StorageMigrationStatus != PictureStorageConstants.MigrationStatusDeletePending) && x.Gallery.User.login == login).Count();
            model.TotalLikesCount = db.Picture.Where(x => (x.StorageMigrationStatus == null || x.StorageMigrationStatus != PictureStorageConstants.MigrationStatusDeletePending) && x.User.Any(y => y.login == login)).Count();
            ...
            model.PictureList = db.Picture
                .Where(x => (x.StorageMigrationStatus == null || x.StorageMigrationStatus != PictureStorageConstants.MigrationStatusDeletePending) && x.Gallery.User.login == login && x.Gallery.LastPicture != null && (!x.Gallery.IsPrivate || (x.Gallery.IsPrivate && x.Gallery.User.login == User.Identity.Name)))
                .OrderByDescending(x => x.id)
                .Take(pictureCount)
            ...
            model.LikeList = db.Picture
                .Where(x => (x.StorageMigrationStatus == null || x.StorageMigrationStatus != PictureStorageConstants.MigrationStatusDeletePending) && x.User.Any(y => y.login == login) && (!x.Gallery.IsPrivate || (x.Gallery.IsPrivate && x.Gallery.User.login == User.Identity.Name)))
                .Take(pictureCount)
                .Select(pic => new PictureModel
                {
                    IdPicture = pic.id,
